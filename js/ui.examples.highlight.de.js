...
(function (root) {
  ...
  function splitSeparableVerb(lemma) {
    ...
    var prefixes = [
      'zurück','zusammen','weiter','zurück','auseinander',
  ...
  function buildGermanForms(lemma, wordObj, deckType) {
    ...
    if (deckType === 'verb') {
      // Проверяем на отделяемый глагол: aufstehen, anrufen, zurückkehren...
      var sep = splitSeparableVerb(base);
      if (sep && sep.base) {
        // Формы основы (stehen, rufen, kehren...), приставку НЕ подсвечиваем
  ...
  function buildVerbForms(lemma) {
    ...
    if (isIeren) {
      ...
      if (hasNoGePrefix) {
        forms.push(stem + 't');    // z.B. "verbündet", "bewegt"
      } else {
