...
(function(root){
  ...
  /* ============================================================
   * PRO FEATURE TOGGLE (MANUAL)
   * ------------------------------------------------------------
   * true  → все PRO-функции включены бесплатно
   * false → обычная логика PRO (платежи / ключи)
   *
   * Когда появится новая платежная система:
   * → просто поставь false
   * ============================================================ */
  var FORCE_PRO_ENABLED = true; // ← МЕНЯТЬ ТОЛЬКО ЭТОТ ФЛАГ

  /* ========================================================
   * Глобальный override PRO-статуса
   * ====================================================== */
  (function(){
    var originalIsPro = A.isPro;

    A.isPro = function(){
      // === ВРЕМЕННЫЙ РЕЖИМ: PRO всегда включён ===
      if (FORCE_PRO_ENABLED) return true;

      // === ОБЫЧНЫЙ РЕЖИМ: старая логика ===
      if (typeof originalIsPro === 'function') {
        try { return originalIsPro(); } catch (_) {}
      }

      try {
        return root.localStorage.getItem('mm.proUnlocked') === '1';
      } catch (_) {
        return false;
      }
    };
