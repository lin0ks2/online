...
(function () {
  ...
  // Определяем текущий ключ деки (нужен для понимания части речи)
  function getCurrentDeckKey() {
    try {
  ...
  // Грубое определение типа слова по ключу деки
  function detectDeckType(deckKey) {
    if (!deckKey) return 'other';
    const k = String(deckKey).toLowerCase();
    if (k.indexOf('verb') !== -1) return 'verb';
    if (k.indexOf('nouns') !== -1 || k.indexOf('noun') !== -1) return 'noun';
    return 'other';
  }

  // Простейшая эвристика стема для глагола
  function guessVerbStem(lemma) {
    let s = String(lemma || '');
    if (!s) return s;

    // глаголы на -ieren оставляем с "ier"
    if (s.endsWith('ieren')) return s.slice(0, -3); // studieren -> studier
  ...
  // Строим набор форм для подсветки
  function buildHighlightForms(lemma, wordObj, deckType) {
    const forms = [];
    ...
    if (wordObj && Array.isArray(wordObj.deForms) && wordObj.deForms.length) {
      wordObj.deForms.forEach(function (f) {
        const v = String(f || '').trim();
        if (v) forms.push(v);
      });
    }

    // 2) Автоматические формы по типу деки
    if (deckType === 'verb') {
      const stem = guessVerbStem(base);
      ...
      // простые множественные/падежные окончания
      forms.push(
        n + 'e',
        n + 'en',
        n + 'er',
    ...
    // 3) Всегда добавляем базовую форму в конце как fallback
    forms.push(base);
  ...
  function highlightSentence(raw, wordObj, deckKey) {
    ...
    const deckType = detectDeckType(deckKey || getCurrentDeckKey());
    const forms = buildHighlightForms(lemma, wordObj, deckType);
    ...
    if (!match || idx < 0) {
      // нет подходящей формы — просто текст без подсветки
      return escapeHtml(raw);
    }

    const before = raw.slice(0, idx);
    const after  = raw.slice(idx + value.length);

    return (
      escapeHtml(before) +
      '<span class="hint-word">' + escapeHtml(value) + '</span>' +
      escapeHtml(after)
    );
  }
  ...
  function renderExamplesTab(word, body) {
    ...
    const deckKey = getCurrentDeckKey();
