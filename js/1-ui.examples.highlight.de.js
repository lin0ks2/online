...
(function (root) {
  ...
  A.ExampleHighlight.de = function (raw, wordObj, deckKey, lemma) {
    ...
    // Собираем набор возможных форм для поиска (включая саму лемму)
  ...
  function buildGermanForms(lemma, wordObj, deckType) {
    ...
    if (deckType === 'verb') {
      ...
      if (sep && sep.base) {
        ...
        // Для Partizip II: приставка + ge + основа (zugestellt, abgemeldet, zurechtgekommen ...)
  ...
  function buildVerbForms(lemma) {
    ...
    // Претерит (очень грубо, в основном для слабых глаголов)
    var prat = [
      stem + 'te',
      stem + 'test',
      stem + 'te',
    ...
    if (isIeren) {
      ...
      if (hasNoGePrefix) {
        forms.push(stem + 't');    // z.B. "bewegt", частично "verstanden" и т.д.
      } else {
  ...
  function buildAdjForms(lemma) {
    ...
    // Базовый набор окончаний прилагательных (сильное/слабое/смешанное склонение)
    forms.push(
      a + 'e',
      a + 'en',
      a + 'em',
