...
(function (root) {
  ...
  // Нерегулярные Partizip II, которые нельзя вывести из стема (для основы)
  var IRREG_PART2_MAP = {
    'gelingen': ['gelungen'],
  ...
  function buildGermanForms(lemma, wordObj, deckType) {
    ...
    if (deckType === 'verb') {
      ...
      var prefix = sep && sep.prefix;
      if (sep && sep.base) {
        // Формы основы (fahren, schließen, kommen, stellen, melden...)
        var baseLemma = sep.base;
        var baseForms = buildVerbForms(baseLemma);
        forms = forms.concat(baseForms);

        // Сильные формы для основы (fahren -> fährt, kommen -> kommt и т.д.)
        var strongEBase = STRONG_E_MAP[baseLemma];
        if (strongEBase) forms = forms.concat(strongEBase);

        var strongABase = STRONG_A_MAP[baseLemma];
        if (strongABase) forms = forms.concat(strongABase);

        // Нерегулярные Partizip II для основы
        var irrP2Base = IRREG_PART2_MAP[baseLemma];
        if (irrP2Base) forms = forms.concat(irrP2Base);
      }

      // Формы самой леммы (abfahren, zustellen, abschließen...)
      forms = forms.concat(buildVerbForms(base));

      // Сильные глаголы — точечные формы Präsens с чередованием гласной для полной леммы
      ...
      // Особые Partizip II для полной леммы (если когда-нибудь понадобятся)
      var irrP2 = IRREG_PART2_MAP[base];
      if (irrP2) forms = forms.concat(irrP2);

      // Если есть приставка — строим формы Partizip II с приставкой:
      // gestellt -> zugestellt, gemeldet -> abgemeldet, geschlossen -> abgeschlossen, gekommen -> zurechtgekommen...
      if (sep && sep.prefix) {
        forms = forms.concat(buildPrefixedPartizip(sep.prefix, forms));
      }
  ...
  // Partizip II с приставкой: ge-Form -> prefix + ge-Form
  // z.B. gestellt -> zugestellt, gemeldet -> abgemeldet, geschlossen -> abgeschlossen, gekommen -> zurechtgekommen
  function buildPrefixedPartizip(prefix, forms) {
    var out = [];
    ...
    for (var i = 0; i < forms.length; i++) {
      ...
      var fl = normalizeNfc(f).toLocaleLowerCase('de-DE');
      // Очень мягкий фильтр: ge...t / ge...en
      if (/^ge.+(t|en)$/.test(fl)) {
        out.push(pre + f);
      }
    }
